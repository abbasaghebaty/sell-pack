import {
  sendMessage,
} from '../api/telegram.js';

import {
  createBlupalInvoice,
} from '../api/blupal.js';

import {
  setUserState,
  clearUserState,
  getUserState,
} from '../database/userStates.js';

import {
  createWalletTopup,
  attachWalletTopupInvoice,
  cancelWalletTopup,
  getWalletTopupById,
} from '../database/walletTopups.js';

import {
  ACCOUNT_BUTTONS,
  getAccountKeyboard,
} from '../../keyboards/account.js';

import {
  createCancelTopupKeyboard,
} from '../../keyboards/walletTopup.js';

const MIN_TOPUP_TOMAN = 10_000;
const MAX_TOPUP_TOMAN = 50_000_000;

function parseAmount(
  value,
) {
  const normalized =
    String(value ?? '')
      .replace(/[,\u066C]/g, '')
      .replace(/[۰-۹]/g, (digit) =>
        String(
          '۰۱۲۳۴۵۶۷۸۹'
            .indexOf(digit),
        ),
      )
      .trim();

  if (!/^\d+$/.test(normalized)) {
    return null;
  }

  const amount =
    Number(normalized);

  if (
    !Number.isSafeInteger(
      amount,
    )
  ) {
    return null;
  }

  return amount;
}

function escapeHtml(value) {
  return String(value ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#039;');
}

export async function startWalletTopup(
  message,
  env,
  db,
) {
  if (!db) {
    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      '❌ دیتابیس در دسترس نیست.',
      getAccountKeyboard(),
    );
  }

  await setUserState(
    db,
    message.from.id,
    'waiting_for_wallet_topup_amount',
    {},
  );

  return sendMessage(
    env.TELEGRAM_BOT_TOKEN,
    message.chat.id,
    `💳 <b>شارژ کیف پول</b>\n\n` +
      `مبلغی که می‌خواهید کیف پولتان را شارژ کنید، به تومان و <b>بدون سه صفر</b> وارد کنید.\n\n` +
      `مثلاً برای شارژ <b>۲۰,۰۰۰ تومان</b> عدد زیر را ارسال کنید:\n\n` +
      `<code>20</code>\n\n` +
      `حداقل مبلغ شارژ: <b>۱۰,۰۰۰ تومان</b>`,
    createCancelTopupKeyboard(),
  );
}

export async function handleWalletTopupAmount(
  message,
  env,
  db,
) {
  const amount =
    parseAmount(
      message.text,
    );

  if (!amount) {
    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      `❌ مبلغ واردشده معتبر نیست.\n\n` +
        `مثلاً برای ۲۰,۰۰۰ تومان فقط بنویسید:\n\n` +
        `<code>20</code>`,
      createCancelTopupKeyboard(),
    );
  }

  if (
    amount <
    MIN_TOPUP_TOMAN
  ) {
    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      `❌ حداقل مبلغ شارژ <b>۱۰,۰۰۰ تومان</b> است.`,
      createCancelTopupKeyboard(),
    );
  }

  if (
    amount >
    MAX_TOPUP_TOMAN
  ) {
    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      `❌ حداکثر مبلغ هر شارژ <b>۵۰,۰۰۰,۰۰۰ تومان</b> است.`,
      createCancelTopupKeyboard(),
    );
  }

  try {
    const topup =
      await createWalletTopup(
        db,
        message.from.id,
        amount,
      );

    let invoice;

    try {
      invoice =
        await createBlupalInvoice(
          env,
          topup.amountRial,
        );
    } catch (error) {
      await cancelWalletTopup(
        db,
        topup.id,
      );

      throw error;
    }

    await attachWalletTopupInvoice(
      db,
      topup.id,
      invoice,
    );

    const expiry =
      new Date(
        topup.expiresAt,
      ).toLocaleString(
        'fa-IR',
      );

    await clearUserState(
      db,
      message.from.id,
    );

    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      `💳 <b>فاکتور شارژ کیف پول</b>\n\n` +
        `مبلغ شارژ کیف پول: <b>${amount.toLocaleString('fa-IR')} تومان</b>\n` +
        `مبلغ قابل پرداخت: <b>${Math.floor(
          invoice.final_amount / 10,
        ).toLocaleString('fa-IR')} تومان</b>\n` +
        `اعتبار فاکتور تا: <b>${escapeHtml(
          expiry,
        )}</b>\n\n` +
        `لطفاً مبلغ دقیق فاکتور را پرداخت کنید.\n\n` +
        `پس از تأیید پرداخت، مبلغ <b>${amount.toLocaleString(
          'fa-IR',
        )} تومان</b> به کیف پول شما اضافه می‌شود.`,
      {
        inline_keyboard: [
          invoice.payment_link
            ? [
                {
                  text:
                    '💳 پرداخت فاکتور',
                  url:
                    invoice.payment_link,
                },
              ]
            : [],
          [
            {
              text:
                '❌ لغو فاکتور',
              callback_data:
                `wallet_topup_cancel:${topup.id}`,
            },
          ],
        ].filter(
          (row) =>
            row.length > 0,
        ),
      },
    );
  } catch (error) {
    console.error(
      'Wallet top-up creation failed:',
      error.message,
      error.stack,
    );

    return sendMessage(
      env.TELEGRAM_BOT_TOKEN,
      message.chat.id,
      `❌ ساخت فاکتور شارژ انجام نشد.\n\n` +
        `<code>${escapeHtml(
          error.message,
        )}</code>`,
      getAccountKeyboard(),
    );
  }
}

export async function cancelWalletTopupFromCallback(
  callbackQuery,
  env,
  db,
) {
  const data =
    String(
      callbackQuery?.data ?? '',
    );

  const topupId =
    Number(
      data.split(':')[1],
    );

  if (
    !Number.isInteger(topupId) ||
    topupId <= 0
  ) {
    return;
  }

  const topup =
    await getWalletTopupById(
      db,
      topupId,
    );

  if (
    !topup ||
    String(topup.telegram_id) !==
      String(
        callbackQuery.from.id,
      )
  ) {
    return;
  }

  await cancelWalletTopup(
    db,
    topupId,
  );

  await clearUserState(
    db,
    callbackQuery.from.id,
  );

  return sendMessage(
    env.TELEGRAM_BOT_TOKEN,
    callbackQuery.message.chat.id,
    '❌ فاکتور شارژ کیف پول لغو شد.',
    getAccountKeyboard(),
  );
}
